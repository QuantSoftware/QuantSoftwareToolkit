...
df_alloc = df_alloc.append( \
             pand.DataFrame(index=[dt.datetime(2010, 2, 3)], \
                              data=[rgen.random()], columns=l_symbols))
...
df_alloc = pand.DataFrame(index=[dt.datetime(2008, 2, 1)], data=[[1,0]], columns=l_symbols)

for i in range(11):
    df_alloc = df_alloc.append( \
             pand.DataFrame(index=[dt.datetime(2009, i+2, 3)], \
                              data=[[rgen.random(),rgen.random()]], columns=l_symbols))
...
report.print_stats(df_aapl, ["$SPX","MSFT"], "AAPL", original=df_goog, d_trading_params={"Number of trades":200,"Trading Period":"Weekly"},d_hedge_params=ls_hedge_params, s_comments="This is a sample report generated by tutorial 5.", leverage=ts_leverage, commissions=f_commision, slippage=f_slippage, directory="./AAPL/")
